...
pub struct HiShelfNode {
    ...
    // but use value in inner.hishelf.format anyway inside new_with_cfg_internal()
    format: Format,
}
...
impl HiShelfNode {
    fn new_with_cfg_internal<N: AsNodeGraphPtr + ?Sized>(
        ...
        n_hishelf_ffi::ma_hishelf_node_init(node_graph, config.as_raw_ptr(), mem.as_mut_ptr())?;
...
pub(crate) mod n_hishelf_ffi {
    ...
    pub fn ma_hishelf_node_uninit(node: &mut HiShelfNode) {
        unsafe {
            sys::ma_hishelf_node_uninit(node.to_raw(), AllocationCallbacks::cb_ptr());
        }
    }
...
impl<'a, N: AsNodeGraphPtr + ?Sized> HiShelfNodeBuilder<'a, N> {
    ...
    pub fn build(&mut self) -> MaResult<HiShelfNode> {
        ...
        HiShelfNode::new_with_cfg_internal(self.node_graph, self)
    }
}
