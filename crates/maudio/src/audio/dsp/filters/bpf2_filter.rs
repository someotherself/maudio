//! Second-order band-pass filtering.
//!
...
//!
//! [`Bpf2`] exposes a Q factor that controls the width of the passband. A higher Q
//! produces a narrower, more resonant response around the cutoff frequency, while a
//! lower Q allows a wider range of frequencies to pass.
//!
//! Use [`Bpf2`] when direct control over the passband width is required. Use [`Bpf`](crate::audio::dsp)
//! when stronger attenuation away from the cutoff frequency is needed and selecting
//! a filter order is more useful than controlling Q directly.
//!
...
//!
//! The sample rate, cutoff frequency, or Q factor can be changed by reinitializing
//! the filter while preserving its internal state. The sample format and
...
impl<F: PcmFormat> Bpf2<F> {
    fn build(config: &sys::ma_bpf2_config, format: Format) -> MaResult<Bpf2<F>> {
        ...
        let mut inner: Box<MaybeUninit<sys::ma_bpf2>> = Box::new(MaybeUninit::uninit());
        bpf2_ffi::ma_bpf2_init(config, inner.as_mut_ptr())?;

        let inner_ptr = Box::into_raw(inner) as *mut sys::ma_bpf2;
        Ok(Bpf2 {
            inner: inner_ptr,
            format,
            channels,
...
pub(crate) mod bpf2_ffi {
    ...
    use crate::{
        audio::dsp::filters::bpf2_filter::Bpf2, pcm_frames::PcmFormat, AllocationCallbacks,
        Binding, MaResult, MaudioError,
    };

    #[inline]
    pub fn ma_bpf2_init(config: &sys::ma_bpf2_config, bpf2: *mut sys::ma_bpf2) -> MaResult<()> {
        let res =
            unsafe { sys::ma_bpf2_init(config as *const _, AllocationCallbacks::cb_ptr(), bpf2) };
    ...
    pub fn ma_bpf2_uninit<F: PcmFormat>(bpf2: &mut Bpf2<F>) {
        unsafe {
            sys::ma_bpf2_uninit(bpf2.to_raw(), AllocationCallbacks::cb_ptr());
        }
    }
